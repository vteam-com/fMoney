...
const double _defaultFuturesFactor = 1.0;
const double _futuresContractMultiplier = 100.0;
...
class SecurityPurchase {
  ...
  double costBasisPerUnit = _zeroDouble;
  ...
  double unitsRemaining = _zeroDouble;

  double get futuresFactor {
    double factor = _defaultFuturesFactor;
    ...
    if (security!.fieldSecurityType.value == SecurityType.futures.index) {
      factor = _futuresContractMultiplier;
    }
    return factor;
  }
  ...
  SecuritySale? sell(DateTime date, double units, double unitSalePrice) {
    if (this.unitsRemaining == _zeroDouble) {
      return null;
    }
