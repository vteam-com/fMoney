...
int? getFirstSelectedMoneyObjectId(List<int> selectedIds) {
...
List<DataObject> getSelectedMoneyObjectsFromIds(
  ...
  return list
      .where(
        (DataObject moneyObject) => selectedIdsSet.contains(moneyObject.uniqueId),
      )
      .toList();
}
...
bool isMoneyObjectMatchingFilters({
  ...
  required Fields<DataObject> fieldToDisplay,
  required DataInterface instance,
...
T? getMoneyObjectFromFirstSelectedIdInList<T>(
  ...
  return listOfItems.firstWhereOrNull(
    (dynamic element) => (element as DataObject).uniqueId == id,
  ) as T?;
}
